...
void ed25519_publickey(const libv_private_key_t privateKey,
    ...
    cx_ecfp_init_private_key(CX_CURVE_Ed25519,
        (uint8_t *)privateKey, sizeof(libv_private_key_t),
        &sdkPrivateKey);
    ...
    os_memmove(publicKey, sdkPublicKey.W+1, sizeof(libv_public_key_t));
}
...
void ed25519_sign(const uint8_t *m, size_t mlen,
    ...
    cx_eddsa_sign(
        ...
        (uint8_t *)m, mlen,
        NULL, 0,
        signature, sizeof(libv_signature_t),
        0);
    os_memset(&sdkPrivateKey, 0, sizeof(sdkPrivateKey));
}
...
int ed25519_sign_open(const uint8_t *m, size_t mlen,
    ...
    os_memmove(sdkPublicKey.W+1, publicKey, sizeof(libv_public_key_t));
    ...
    return cx_eddsa_verify(
        ...
        (uint8_t *)m, mlen,
        NULL, 0,
        (uint8_t *)signature, sizeof(libv_signature_t));
}
