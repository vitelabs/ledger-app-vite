...
void app_dispatch(void) {
    ...
    uint16_t statusWord;
    uint32_t apduHash;
    libv_apdu_response_t *resp = &libv_context_D.response;
    ...
    BEGIN_TRY_L(dispatch) {
        TRY_L(dispatch) {
            ...
            SB_CHECK(libv_context_D.halted);
            if (SB_GET(libv_context_D.halted)) {
                statusWord = LIBV_SW_HALTED;
                goto sendSW;
            }

#ifdef HAVE_IO_U2F
            if (G_io_apdu_state == APDU_U2F) {
                apduHash = libv_simple_hash(G_io_apdu_buffer, libv_context_D.inLength);
                if (apduHash == libv_context_D.u2fRequestHash) {
                    if (libv_context_D.state != LIBV_STATE_READY) {
                        // Request ongoing, setup a timeout
                        libv_context_D.u2fTimeout = U2F_REQUEST_TIMEOUT;

                        resp->ioFlags |= IO_ASYNCH_REPLY;
                        statusWord = LIBV_SW_OK;
                        goto sendSW;

                    } else if (libv_context_D.stateData.asyncResponse.outLength > 0) {
            ...
            if (dispatched == DISPATCHER_APDUS) {
                statusWord = LIBV_SW_INS_NOT_SUPPORTED;
                goto sendSW;
            }
            if (DISPATCHER_DATA_IN[dispatched]) {
                if (G_io_apdu_buffer[ISO_OFFSET_LC] == 0x00 ||
                    libv_context_D.inLength - 5 == 0) {
                    statusWord = LIBV_SW_INCORRECT_LENGTH;
                    goto sendSW;
                }
            ...
            if (G_io_apdu_state == APDU_U2F && (resp->ioFlags & IO_ASYNCH_REPLY) != 0) {
                ...
                libv_context_D.u2fRequestHash = apduHash;
                libv_context_D.u2fTimeout = U2F_REQUEST_TIMEOUT;
            }
#endif // HAVE_IO_U2F
        ...
        CATCH_OTHER_L(dispatch, e) {
            ...
            SB_SET(libv_context_D.halted, 1);
        }
        FINALLY_L(dispatch);
    }
    END_TRY_L(dispatch);
...
void app_async_response(libv_apdu_response_t *resp, uint16_t statusWord) {
    resp->buffer[resp->outLength] = (statusWord >> 8);
    resp->buffer[resp->outLength + 1] = (statusWord & 0xff);
    resp->outLength += 2;
    ...
    os_memmove(&libv_context_D.stateData.asyncResponse, resp, sizeof(libv_apdu_response_t));
    app_apply_state();
}

bool app_send_async_response(libv_apdu_response_t *resp) {
#ifdef HAVE_IO_U2F
    if (G_io_apdu_state == APDU_IDLE) {
        return false;
    }

    libv_context_D.u2fTimeout = 0;
#endif // HAVE_IO_U2F
    ...
    io_exchange(CHANNEL_APDU | IO_RETURN_AFTER_TX,
        libv_context_D.response.outLength);
    return true;
}
...
bool app_apply_state(void) {
    ...
    bool uxChanged = libv_bagl_apply_state();
    if (uxChanged) {
        return true;
    }
    ...
    if (libv_context_D.state == LIBV_STATE_READY &&
        libv_context_D.stateData.asyncResponse.outLength > 0) {
        bool responseSent = app_send_async_response(&libv_context_D.stateData.asyncResponse);
        if (responseSent) {
            return true;
        }
    }
...
void app_main(void) {
    os_memset(libv_context_D.response.buffer, 0, 255); // paranoia

    // Process the incoming APDUs

    // first exchange, no out length :) only wait the apdu
    libv_context_D.response.outLength = 0;
    libv_context_D.response.ioFlags = 0;
    for (;;) {
        ...
        libv_context_D.inLength =
            io_exchange(CHANNEL_APDU | libv_context_D.response.ioFlags,
...
#ifdef HAVE_IO_U2F
        if (libv_context_D.u2fTimeout > 0) {
            libv_context_D.u2fTimeout -= MIN(100, libv_context_D.u2fTimeout);
            if (libv_context_D.u2fTimeout == 0) {
                u2f_message_timeout();
                break;
            }
